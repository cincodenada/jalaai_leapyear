...
ggplot() +
    ...
    geom_segment(data=cycles, aes(x=start,xend=start,y=ypos+ticksize,yend=ypos), color=scalecol) +
    geom_segment(data=cycles, aes(x=end,xend=end,y=ypos+ticksize,yend=ypos), color=scalecol) +
    geom_text(data=cycles, aes(label=len, x=mid), y=ypos-ticksize*1.5, color=scalecol, size=3) +
